...
class InlineCompletionHandler(JupyterHandler, websocket.WebSocketHandler):
    """Receives completion requests and dispatches them to the model supporting infill."""

    @property
    def websocket_sessions(self) -> Dict[str, "InlineCompletionHandler"]:
        """Mapping sessions IDs to their corresponding InlineCompletionHandler instances."""
        return self.settings["jai_inline_completion_sessions"]
    ...
    def open(self):
        ...
        self.websocket_sessions[client_id] = self
        self.client_id = client_id
        self.write_message(ConnectionMessage(client_id=client_id).dict())
    ...
    async def _complete(self, request: InlineCompletionRequest):
        start = time.time()
        reply = await self.settings["jai_inline_completion_handler"].on_message(request)
        latency_ms = round((time.time() - start) * 1000)
    ...
    def on_close(self):
        ...
        self.log.info(f"Completer session disconnected. ID: {self.client_id}")
